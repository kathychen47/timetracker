...
T("记下来的都是「发生过的事」，不再问她确认一遍", function () {
  // 勾拿掉之后，done 就不再是个需要她抬手的东西。
  // 但字段得留着：旧记录里有它，evIsDemo 也靠它认「这条是真数据」。
  ok(count("done:doneDefault") === 0, "没有地方再读那个设置了");
  ok(whole.indexOf('cat:ck,sub:null,done:true};') >= 0, "AI 建的事件直接记成发生过了");
  ok(count("done:true,par:!!(pf&&pf.checked)") === 1, "番茄钟落库的一直就是");
  ok(whole.indexOf("var isDone=editingId?") >= 0, "改旧条时原样保留它自己的值，不被重置");
});
